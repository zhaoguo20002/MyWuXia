	    ...
	    void OnGUI () {
			...
			if (listNames != null && listNames.Count > 0) {
				...
				if (data != null) {
					...
					showId = data.Id;
					GUI.Label(new Rect(65, 0, 60, 18), "名称:");
					srcName = EditorGUI.TextField(new Rect(130, 0, 100, 18), srcName);
					GUI.Label(new Rect(65, 20, 60, 18), "Id:");
					showId = EditorGUI.TextField(new Rect(130, 20, 100, 18), showId);
					...
					GUI.Label(new Rect(0, 20, 60, 18), "名称:");
					addSrcName = EditorGUI.TextField(new Rect(65, 20, 160, 18), addSrcName);
					GUI.Label(new Rect(0, 40, 60, 18), "Id:");
					addId = EditorGUI.TextField(new Rect(65, 40, 160, 18), addId);
					addSprite = EditorGUI.ObjectField(new Rect(0, 60, 225, 60), "添加头像IconId", addSprite, typeof(Sprite), true) as Sprite;
					if (GUI.Button(new Rect(0, 125, 80, 36), "添加")) {
						if (addSrcName == "") {
							this.ShowNotification(new GUIContent("名称不能为空!"));
							return;
						}
						...
						writeDataToJson();
						selGridInt = srcDataMapping.Count - 1;
						getData();
						fetchData(searchKeyword);
						addId = "";
						addSrcName = "";
